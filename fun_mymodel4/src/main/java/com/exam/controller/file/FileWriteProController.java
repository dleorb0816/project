...
public class FileWriteProController implements Controller {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		fileVo.setReadcount(0);  // 조회수

		//re_ref  re_lev  re_seq
		fileVo.setReRef(nextNum); // 주글일때는 글번호가 그룹번호가 됨
		fileVo.setReLev(0); // 주글일때는 들여쓰기 레벨이 0 (들여쓰기 없음)
		fileVo.setReSeq(0); // 주글일때는 글그룹 내에서 순번이 0 (첫번째)
