            ...
            <div class="row">
                <div class="col-3"></div>
                	<div class="col-3">
							...
							<table id="file" class="table table-striped" >
								<tr>
								...
								<c:choose>
								<c:when test="${ not empty fileList }"><%-- ${ pageDto.count gt 0 } --%>

									<c:forEach var="file" items="${ fileList }">
										<tr>
											<td>${ file.num }</td>
											<td class="left">
												<c:if test="${ file.reLev gt 0 }"><%-- 답글이면 --%>
													<img src="../images/level.gif" width="${ file.reLev * 15 }" height="13">
													<img src="../images/re.gif">
												</c:if>
												<a href="fileContent.do?num=${ file.num }&pageNum=${ pageNum }">${ file.subject }</a>
											</td>
											<td>${ file.id }</td>
											<td><fmt:formatDate value="${ file.regDate }" pattern="yyyy.MM.dd"/></td>
											<td>${ file.readcount }</td>
										</tr>
									</c:forEach>
							...
							<div id="table_search">
								<form action="fileBoard.do" method="get">
							...
							<c:if test="${ pageDto.count gt 0 }">
								...
								<c:if test="${ pageDto.startPage gt pageDto.pageBlock }">
									<a href="fileBoard.do?pageNum=${ pageDto.startPage - pageDto.pageBlock }&category=${ pageDto.category }&search=${ pageDto.search }">[이전]</a>
								</c:if>
								...
								<c:forEach var="i" begin="${ pageDto.startPage }" end="${ pageDto.endPage }" step="1">
									...
									<c:when test="${ i eq pageNum }">
										<a href="fileBoard.do?pageNum=${ i }&category=${ pageDto.category }&search=${ pageDto.search }" class="active">[${ i }]</a>
									</c:when>
									<c:otherwise>
										<a href="fileBoard.do?pageNum=${ i }&category=${ pageDto.category }&search=${ pageDto.search }">[${ i }]</a>
									</c:otherwise>
									</c:choose>
								...
								<c:if test="${ pageDto.endPage lt pageDto.pageCount }">
									<a href="fileBoard.do?pageNum=${ pageDto.startPage + pageDto.pageBlock }&category=${ pageDto.category }&search=${ pageDto.search }">[다음]</a>
								</c:if>
							</c:if>
