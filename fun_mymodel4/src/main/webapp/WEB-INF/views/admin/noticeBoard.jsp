        ...
        <div class="container" align="center">
            <div class="row">
                	...
                	<div class="col-3">
                      		...
                      		<h1>공지사항 [글갯수: ${ pageDto.count }]</h1>
							<table id="notice" class="tabe table-striped" style="width: 695px">
										...
										<tr style="height: 56px">
											...
											<td class="left">
												<c:if test="${ notice.reLev gt 0 }"><%-- 답글이면 --%>
													<img src="/img/level.gif" width="${ notice.reLev * 15 }" height="13">
													<img src="/img/re.gif">
												</c:if>
												<a href="adminNoticeContent.do?num=${ notice.num }&pageNum=${ pageNum }">${ notice.subject }</a>
											</td>
											<td>${ notice.id }</td>
							...
							<div id="table_search">
								<form action="adminNoticeBoard.do" method="get">
									...
									<input type="text" class="input_box" name="search" value="${ pageDto.search }" style="width: 528px">
									<input type="submit" value="검색" class="btn">
									
									<%-- 관리자만 [글쓰기] 버튼 보이기 --%>
									<c:if test="${ level eq '관리자' }">
										<input type="button" value="파일글쓰기" class="btn" onclick="location.href='adminNoticeWriteForm.do?pageNum=${ pageNum }'">
									</c:if>
							...
							<c:if test="${ pageDto.count gt 0 }">
								...
								<c:if test="${ pageDto.startPage gt pageDto.pageBlock }">
									<a href="adminNoticeBoard.do?pageNum=${ pageDto.startPage - pageDto.pageBlock }&category=${ pageDto.category }&search=${ pageDto.search }">[이전]</a>
								</c:if>
								...
								<c:forEach var="i" begin="${ pageDto.startPage }" end="${ pageDto.endPage }" step="1">
									...
									<c:when test="${ i eq pageNum }">
										<a href="adminNoticeBoard.do?pageNum=${ i }&category=${ pageDto.category }&search=${ pageDto.search }" class="active">[${ i }]</a>
									</c:when>
									<c:otherwise>
										<a href="adminNoticeBoard.do?pageNum=${ i }&category=${ pageDto.category }&search=${ pageDto.search }">[${ i }]</a>
									</c:otherwise>
									</c:choose>
								...
								<c:if test="${ pageDto.endPage lt pageDto.pageCount }">
									<a href="adminNoticeBoard.do?pageNum=${ pageDto.startPage + pageDto.pageBlock }&category=${ pageDto.category }&search=${ pageDto.search }">[다음]</a>
								</c:if>
							</c:if>
